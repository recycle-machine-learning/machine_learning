...
from project.datatransform.image_resize import resize_dataset_image
...
def load_data():
    ...
    with mp.Pool(processes=mp.cpu_count()) as pool:
        ...
        pool_result = pool.starmap(load_data_single_class,
                                   zip(file_list, [garbage_class.index(file) for file in file_list]))
...
def load_data_single_class(path, class_idx):
    image_file = resize_dataset_image(path='dataset/garbage_classification/' + path, target_height=64, target_width=64)
